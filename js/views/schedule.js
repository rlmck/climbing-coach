...
(function () {
  ...
  /* How long a press has to last before it stops being a tap. A press
     held this long is a hand picking the tile up to move it, and letting
     go of it again should leave the screen as it was — not open a sheet
    ...
    function slotNode(slot) {
      ...
      const node = el('div', {
        ...
        title: locked ? 'Open to edit or delete'
                      : 'Tap to log or remove · hold and drag to another day'
      }, [
      ...
      if (locked) {
        ...
        /* The tap is recognised from the pointer events themselves rather
           ...
           that travelled, or was held, is a hand moving the tile. None of
           that has to be guessed from a click that may or may not come. */
    ...
    function render() {
      ...
      shell.appendChild(el('div', { class: 'card', style: 'padding:14px 20px;display:flex;gap:22px;flex-wrap:wrap;align-items:center' }, [
        ...
        el('p', { class: 'tiny', style: 'margin-left:auto',
          text: hasDrag ? 'Tap a session to log or remove it · hold and drag to move it'
    ...
    function setupDrag() {
      CT.ui.$$('.slot', shell).forEach(node => {
        ...
        draggables.push(Draggable.create(node, {
          ...
          onDragEnd() {
            /* Decided before anything resets the transform. */
            const target = dayUnderPointer(this.pointerX, this.pointerY);
